...
export default function CustomInvoiceGenerator({
  ...
  // Catalog search filtering: Instant local match + Debounced backend API lookup across all 2,200+ medicines
  useEffect(() => {
    const query = catalogSearch.trim();
    if (!query) {
      // Default to initial suggestions from products prop
      setCatalogSearchResults(products.slice(0, 20));
    ...
    // 1. Instant local match for instantaneous 0ms feedback
    ...
    if (localMatches.length > 0) {
      setCatalogSearchResults(localMatches.slice(0, 30));
    }

    // 2. Query backend catalog API to search entire 2,200+ product master registry
    setIsSearchingCatalog(true);
    const reqId = ++searchRequestIdRef.current;

    const timer = setTimeout(async () => {
      try {
        const fetched = await productService.getProducts({ search: query, limit: 50 });
        if (searchRequestIdRef.current === reqId) {
          const list = Array.isArray(fetched) ? fetched : [];
          // Merge local and fetched results, deduplicating by ID
          ...
          setCatalogSearchResults(combined);
          setIsSearchingCatalog(false);
        }
      } catch (err) {
        if (searchRequestIdRef.current === reqId) {
          console.error("Failed to query catalog medicines", err);
          setIsSearchingCatalog(false);
        }
      }
    }, 150);
  ...
  // Add custom item with name from search
  const handleAddCustomItemWithName = (name: string) => {
    const trimmed = name.trim();
    const newItem: CustomInvoiceItem = {
      id: `item-${Date.now()}-${Math.random().toString(36).slice(2, 5)}`,
      name: trimmed || "Custom Medicine / Hospital Supply",
      category: "Tablet",
      strength: "",
      packSize: "Unit",
      mrp: 100,
              ...
              <div className="relative" ref={catalogSearchRef}>
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Search MediChain catalog to quick-add medicines (e.g. Lorix, Napa, Seclo, Maxpro)..."
                    className="w-full text-xs pl-9 pr-16 py-2.5 bg-white text-slate-900 border border-purple-300 rounded-xl focus:ring-2 focus:ring-purple-500/20 focus:border-purple-600 outline-none shadow-xs font-medium"
                  />
                  {isSearchingCatalog && (
                    <Loader2 className="w-4 h-4 text-purple-600 animate-spin absolute right-9 top-3 pointer-events-none" />
                  )}
                  {catalogSearch && (
                    <button
                      type="button"
                ...
                {/* Dropdown search results across entire catalog */}
                {showCatalogDropdown && (
                  <div className="absolute top-full left-0 right-0 mt-1.5 bg-white border border-slate-200 rounded-xl shadow-2xl z-50 max-h-80 overflow-y-auto divide-y divide-slate-100 animate-fade-in">
                    <div className="p-2.5 bg-slate-50 text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center justify-between sticky top-0 z-10 border-b border-slate-100">
                      <span>
                        {catalogSearch.trim()
                          ? `Search Results (${catalogSearchResults.length} found)`
                          : "Suggested Catalog Medicines"}
                      </span>
                      {isSearchingCatalog ? (
                        <span className="flex items-center gap-1 text-purple-600 font-bold normal-case text-[11px]">
                          <Loader2 className="w-3 h-3 animate-spin" />
                          Searching catalog...
                        </span>
                      ) : (
                        <span className="text-slate-400 font-normal text-[10px] lowercase">
                          {catalogSearch.trim() ? "live catalog matches" : "2,200+ available medicines"}
                        </span>
                      )}
                    </div>

                    {/* Active search skeleton/loading */}
                    {isSearchingCatalog && catalogSearchResults.length === 0 && (
                      <div className="p-6 text-center text-xs text-purple-700 flex flex-col items-center justify-center gap-2">
                        <Loader2 className="w-5 h-5 animate-spin text-purple-600" />
                        <span className="font-semibold">Searching catalog for "{catalogSearch}"...</span>
                      </div>
                    )}
                    ...
                    {catalogSearchResults.map(prod => (
                      <div
                        ...
                        className="p-2.5 sm:p-3 hover:bg-purple-50/80 transition-colors cursor-pointer flex items-center justify-between gap-3 text-xs group"
                      >
                        <div className="min-w-0 flex-1">
                          <div className="font-bold text-slate-900 flex items-center gap-1.5 flex-wrap">
                            <span className="text-slate-900 group-hover:text-purple-700 transition-colors font-semibold">
                              {prod.name}
                            </span>
                            {prod.strength && (
                              <span className="text-[10.5px] text-purple-700 font-medium bg-purple-50 px-1.5 py-0.5 rounded border border-purple-100">
                                {prod.strength}
                              </span>
                            )}
                            <span className="text-[9.5px] font-bold text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded uppercase tracking-wide">
                              {resolveItemType(prod)}
                            </span>
                          </div>
                          <div className="text-[11px] text-slate-500 mt-0.5 truncate">
                            {prod.genericName ? <span className="font-medium text-slate-700">{prod.genericName}</span> : "Generic"}
                        ...
                        <div className="text-right shrink-0 flex items-center gap-2.5">
                          ...
                          <div className="hidden sm:flex items-center justify-center w-7 h-7 rounded-lg bg-purple-100 text-purple-700 group-hover:bg-purple-600 group-hover:text-white transition-colors">
                    ...
                    {/* No matches fallback with custom item option */}
                    {!isSearchingCatalog && catalogSearch.trim() && catalogSearchResults.length === 0 && (
                      <div className="p-5 text-center space-y-2.5">
                        <p className="text-xs font-semibold text-slate-700">
                          No medicine found matching "<span className="text-purple-700 font-bold">{catalogSearch}</span>" in catalog
                        </p>
                        <p className="text-[11px] text-slate-400 max-w-sm mx-auto">
                          You can still add this medicine or surgical product as a custom line item on this invoice.
                        </p>
                        <button
                          type="button"
                          onMouseDown={e => {
                            e.preventDefault();
                            handleAddCustomItemWithName(catalogSearch);
                          }}
                          className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold shadow-xs cursor-pointer transition-all"
                        >
                          <Plus className="w-4 h-4" />
                          <span>Add "{catalogSearch}" as Custom Item</span>
                        </button>
                      </div>
                    )}
                  </div>
