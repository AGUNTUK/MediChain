...
export default function AdminPanel({ currentUser, onLogout }: AdminPanelProps) {
  ...
  const fetchPriceHistory = async (productId: string) => {
    try {
      const res = await apiFetch(`/api/admin/products/${productId}/price-history`);
      if (res.ok) {
  ...
  const syncInventoryAlerts = async () => {
    try {
      const res = await apiFetch("/api/admin/inventory/alerts/sync", { method: "POST" });
      if (res.ok) {
        const data = await res.json();
  ...
  const recordExportHistory = async (type: string, format: string, filters: any) => {
    try {
      await apiFetch("/api/admin/export-history", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ type, format, filters })
      });
      // reload history
      const expRes = await apiFetch("/api/admin/export-history");
      if (expRes.ok) {
  ...
  const refreshAllData = async () => {
    ...
    try {
      const results = await Promise.allSettled([
        ...
        apiFetch("/api/admin/invoices").then(r => r.ok ? r.json() : null),
        apiFetch("/api/admin/export-history").then(r => r.ok ? r.json() : null),
        apiFetch("/api/admin/analytics").then(r => r.ok ? r.json() : null),
        apiFetch("/api/admin/audit-logs").then(r => r.ok ? r.json() : null),
        apiFetch("/api/admin/import-history").then(r => r.ok ? r.json() : null),
        apiFetch("/api/admin/finance/summary").then(r => r.ok ? r.json() : null),
        apiFetch("/api/admin/notifications").then(r => r.ok ? r.json() : null),
      ...
      if (dashRes.status === "fulfilled" && dashRes.value?.metrics) {
        setTotalProductsCount(dashRes.value.metrics.totalProducts || 0);
      }

      if (paginatedRes.status === "fulfilled" && paginatedRes.value) {
      ...
      if (pharmData.status === "fulfilled" && pharmData.value) {
        const list = Array.isArray(pharmData.value) 
          ? pharmData.value 
          : (pharmData.value.pharmacies || []);
        setPharmacies(list);
      }
      ...
      if (invData.status === "fulfilled" && invData.value) {
        setInvoices(invData.value.invoices || []);
      }

      if (expData.status === "fulfilled" && expData.value) {
        setExportHistory(expData.value.history || []);
      }

      if (analData.status === "fulfilled" && analData.value) {
        setAnalyticsData(analData.value);
      }

      if (auditData.status === "fulfilled" && auditData.value) {
        setAuditLogs(auditData.value.auditLogs || []);
      }

      if (importHistData.status === "fulfilled" && importHistData.value) {
        setImportHistory(importHistData.value.history || []);
      }

      if (financeData.status === "fulfilled" && financeData.value) {
        setFinanceSummary(financeData.value);
        setPaymentLedger(financeData.value.paymentHistory || []);
      }

      if (notifHistData.status === "fulfilled" && notifHistData.value) {
        setNotifHistory(notifHistData.value.history || []);
      }

      if (rMetrics.status === "fulfilled" && rMetrics.value) {
        setPendingRestockCount(rMetrics.value.totalPendingRequests || 0);
      }
    } catch (err: any) {
  ...
  const handleSaveProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedProductForEdit) {
        const res = await apiFetch(`/api/admin/products/${selectedProductForEdit.id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      await apiFetch("/api/admin/import-history", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleSaveInventoryRow = async (id: string) => {
    ...
    try {
      const res = await apiFetch("/api/admin/inventory/update", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          id,
  ...
  const [adminAmendingItem, setAdminAmendingItem] = useState<{ productId: string; name: string; quantity: number } | null>(null);
  const [adminAmendReason, setAdminAmendReason] = useState("");
  const [isAdminAmending, setIsAdminAmending] = useState(false);

  const handleAdminAmendItem = async () => {
    if (!adminAmendingItem || !selectedOrderDetails) return;
    setIsAdminAmending(true);
    try {
      const res = await orderService.amendOrderLineItem(
        selectedOrderDetails.id,
        adminAmendingItem.productId,
        adminAmendReason
      );
      if (res.success && res.order) {
        setSelectedOrderDetails(res.order);
        setSuccessMsg(`Amended order: removed ${adminAmendingItem.name}. Invoice recalculated.`);
        setAdminAmendingItem(null);
        setAdminAmendReason("");
        refreshAllData();
      }
    } catch (err: any) {
      setErrorMsg(err.message || "Failed to mark item unavailable.");
    } finally {
      setIsAdminAmending(false);
    }
  };
  ...
  const handleExportData = async (format: "excel" | "json") => {
    try {
      ...
      const logsRes = await apiFetch("/api/admin/export-history");
      if (logsRes.ok) {
  ...
  const handleUpdatePharmacyStatus = async (pharmacyId: string, status: string) => {
    try {
      const res = await apiFetch(`/api/admin/pharmacies/${pharmacyId}/status`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen w-screen bg-slate-50 text-slate-900 font-sans overflow-hidden">
      ...
      `}>
        ...
        <div className="p-6 border-t border-slate-900 bg-white/60 flex items-center justify-between flex-shrink-0">
          <div className="truncate pr-2">
            <span className="text-xs font-bold text-slate-900 block truncate">{currentUser?.name || "Administrator"}</span>
            <span className="text-[10px] text-slate-500 font-mono block truncate">{currentUser?.phone || "Admin Profile"}</span>
          </div>
          <button 
            onClick={onLogout}
            title="Sign Out"
                                ...
                                <td className="p-4">
                                  <div className="flex items-center gap-3">
                                    <div className="w-8 h-8 rounded-lg bg-slate-50 border border-slate-200 flex items-center justify-center text-[10px] text-indigo-400 font-extrabold uppercase overflow-hidden">
                                      {p.imageUrl || p.image_url ? (
                                        <img src={p.imageUrl || p.image_url} alt={p.name} loading="lazy" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                                      ) : (
                                        p.name.slice(0, 2)
                                      )}
                                    </div>
      ...
      {/* Admin Order Line Item Amendment Modal */}
      {adminAmendingItem && selectedOrderDetails && (
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4 animate-scale-up text-white">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-extrabold text-sm text-white">Mark Line Item Unavailable</h3>
                  <p className="text-[10px] text-slate-400">Wholesale Procurement Adjustment</p>
                </div>
              </div>
              <button onClick={() => setAdminAmendingItem(null)} className="p-1 rounded-lg hover:bg-slate-800 text-slate-400">
                <X className="w-4 h-4" />
              </button>
            </div>

            <div className="bg-slate-950 p-3 rounded-xl border border-slate-800 text-xs space-y-1">
              <p className="font-extrabold text-slate-100">{adminAmendingItem.name}</p>
              <div className="flex justify-between items-center pt-1 border-t border-slate-800 font-mono text-[11px] text-slate-300">
                <span>Removing quantity: x{adminAmendingItem.quantity} units</span>
                <span className="font-bold text-rose-400">Order #{selectedOrderDetails.id.slice(0, 8)}</span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <input
                type="text"
                value={adminAmendReason}
                onChange={(e) => setAdminAmendReason(e.target.value)}
                placeholder="যেমন: হোলসেলার স্টকে নেই / সাময়িক অনুপলব্ধ"
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-indigo-500"
              />
            </div>

            <div className="text-[11px] text-slate-400 bg-indigo-950/40 border border-indigo-900/40 rounded-xl p-3">
            ...
            <div className="flex gap-2 pt-2">
              <button
                type="button"
                onClick={() => setAdminAmendingItem(null)}
                disabled={isAdminAmending}
                className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold py-2 px-4 rounded-xl text-xs cursor-pointer"
              >
                Cancel
              </button>
              <button
                type="button"
                onClick={handleAdminAmendItem}
                disabled={isAdminAmending}
              ...
              >
                {isAdminAmending ? "Recalculating..." : "Confirm & Amend Order"}
              </button>
            </div>
          </div>
        </div>
