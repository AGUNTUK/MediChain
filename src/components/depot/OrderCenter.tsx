...
import { 
  ...
  Layers, CheckSquare, Square, AlertCircle, Barcode, Scan, KeyRound, ShieldCheck, Download,
  Ban, History
} from "lucide-react";
...
export default function OrderCenter({ 
  ...
  const [activeTab, setActiveTab] = useState<"all" | "pending" | "confirmed" | "processing" | "packed" | "dispatch" | "delivered">("all");
  ...
  // Order Amendment State (Operational unavailability before Packed)
  const [amendingItem, setAmendingItem] = useState<{ item: OrderItem; orderId: string } | null>(null);
  const [amendReason, setAmendReason] = useState("");
  const [isAmending, setIsAmending] = useState(false);
  const [amendError, setAmendError] = useState("");

  const handleConfirmAmend = async () => {
    if (!amendingItem || !selectedOrder) return;
    setIsAmending(true);
    setAmendError("");
    try {
      const res = await orderService.amendOrderLineItem(
        selectedOrder.id,
        amendingItem.item.productId,
        amendReason
      );
      if (res.success && res.order) {
        setSelectedOrder(res.order);
        // Clear picked state for this item if set
        const itemKey = `${amendingItem.item.productId}_${amendingItem.item.name}`;
      ...
      setAmendError(err.message || "Failed to mark item unavailable.");
    } finally {
      setIsAmending(false);
    }
  };
  ...
  const filteredOrders = orders.filter(order => {
    ...
    if (activeTab === "dispatch" && order.status !== "Out for Delivery") return false;
  ...
  const handleDownloadSlip = () => {
    ...
    const slipId = `PK-${selectedOrder.id.substring(0, 8).toUpperCase()}`;
    const dateStr = new Date().toLocaleDateString();
    const pharmacyStr = selectedOrder.pharmacyName || `Pharmacy ID ${selectedOrder.pharmacyId?.substring(0, 8) || ""}`;
    const addressStr = selectedOrder.deliveryAddress || "Standard Route";
    
    const optimizedItems = getOptimizedPickingPath(selectedOrder.items || []);
    let itemsText = "";
    optimizedItems.forEach((item, idx) => {
      const loc = getRackLocation(item.productId, item.name, "Tablet");
      itemsText += `${idx + 1}. ${item.name}  x${item.quantity}\n   Shelf: ${loc}  |  ${item.strength || "Standard"}\n`;
    });

    const slipContent = `================================================
               MEDICHAIN DEPOT
          WAREHOUSE MANAGEMENT SLIP
            Dhaka Main Warehouse-01
================================================
SLIP ID:    ${slipId}
ORDER ID:   ${selectedOrder.readableId || selectedOrder.id}
DATE:       ${dateStr}
DELIVER TO: ${pharmacyStr}
ADDRESS:    ${addressStr}
...
TOTAL VALUE: ৳${selectedOrder.totalAmount}
================================================
[ QR CODE VERIFICATION: PK-${selectedOrder.id.substring(0, 8).toUpperCase()} ]

Picker Sign: ___________________________
Rider Sign:  ___________________________
================================================
`;
    ...
    link.href = url;
    link.download = `PackingSlip-${slipId}.txt`;
  ...
  const handleDownloadInvoicePdf = () => {
    if (!selectedOrder) return;
    const downloadUrl = `/api/orders/${selectedOrder.id}/invoice`;
    const link = document.createElement("a");
    link.href = downloadUrl;
    link.download = `Invoice-${selectedOrder.readableId || selectedOrder.id.substring(0, 8)}.pdf`;
    link.target = "_blank";
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
      ...
      <div className="flex flex-col md:flex-row justify-between items-stretch md:items-center gap-4 bg-white dark:bg-slate-950 p-4 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm">
        ...
        <div className="flex gap-1 bg-slate-100 dark:bg-slate-900 p-1 rounded-xl border border-slate-200 dark:border-slate-800 shrink-0 overflow-x-auto">
          {[
            ...
            { id: "dispatch", label: "In Transit", count: orders.filter(o => o.status === "Out for Delivery").length },
            { id: "delivered", label: "Delivered / Past", count: orders.filter(o => o.status === "Delivered").length }
          ].map(tab => (
            <button
              key={tab.id}
        ...
        <div className="fixed inset-0 bg-slate-950/70 dark:bg-slate-950/85 backdrop-blur-sm flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto text-xs">
                      ...
                      <div key={idx} className={`p-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 transition-all ${
                        ...
                        <div className="flex items-center justify-between sm:justify-end gap-3 border-t sm:border-t-0 border-slate-100 dark:border-slate-900 pt-2 sm:pt-0">
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            {/* Procurement Line Item Amendment Action (Allowed through Packed stage) */}
                            {["Pending", "Confirmed", "Processing", "Packed"].includes(selectedOrder.status) && (
                              <button
                                type="button"
                                onClick={() => {
                                  setAmendingItem({ item, orderId: selectedOrder.id });
                                  setAmendReason("হোলসেলার স্টকে সাময়িক অনুপলব্ধ");
                                  setAmendError("");
                                }}
                                className="px-2.5 py-1.5 rounded-xl text-[10px] font-extrabold transition-all cursor-pointer flex items-center gap-1.5 bg-rose-50 hover:bg-rose-100 dark:bg-rose-950/50 dark:hover:bg-rose-900/70 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800 shadow-3xs hover:shadow-2xs"
              ...
              {/* Transit Lockdown Notice */}
              {!["Pending", "Confirmed", "Processing", "Packed"].includes(selectedOrder.status) && (
                <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2.5 text-[11px] text-slate-600 dark:text-slate-400 font-medium flex items-center gap-2">
                  <ShieldCheck className="w-4 h-4 text-emerald-600 shrink-0" />
                  <span>Order is <strong>{selectedOrder.status}</strong> (Dispatched/Finalized) — Line items are locked against further procurement amendments.</span>
                </div>
              )}

              {/* Procurement Amendment Audit History */}
              {selectedOrder.amendments && selectedOrder.amendments.length > 0 && (
                <div className="bg-amber-50/70 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/50 rounded-xl p-3.5 space-y-2">
                  <div className="flex items-center justify-between">
                    <h4 className="text-[10px] font-black uppercase text-amber-800 dark:text-amber-400 flex items-center gap-1.5 tracking-wider">
                      <History className="w-3.5 h-3.5 text-amber-600" />
                      Procurement Amendment History ({selectedOrder.amendments.length} Items Adjusted)
                    </h4>
                    <span className="text-[9px] text-amber-700 dark:text-amber-300 font-bold bg-amber-100 dark:bg-amber-900/40 px-2 py-0.5 rounded">COD Auto-Recalculated</span>
                  </div>
                  <div className="divide-y divide-amber-200/60 dark:divide-amber-900/40">
                    {selectedOrder.amendments.map((amend, aIdx) => (
                      <div key={aIdx} className="py-2 flex items-start justify-between gap-2 text-xs">
                        <div>
                          <p className="font-extrabold text-amber-900 dark:text-amber-200">
                            {amend.productName} <span className="text-amber-700 dark:text-amber-400 font-semibold">(-{amend.removedQuantity} Qty)</span>
                          </p>
                          <p className="text-[10px] text-amber-800/80 dark:text-amber-400/80 italic mt-0.5">
                            Reason: "{amend.reason || "Wholesaler stock unavailable"}"
                          </p>
                        </div>
                        <div className="text-right text-[10px] text-amber-700 dark:text-amber-400 font-medium">
                          <span>By {amend.amendedBy}</span>
                          <span className="block text-[9px] text-amber-600/70">{new Date(amend.amendedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {/* Procurement Line Item Amendment Modal */}
              {amendingItem && (
                  ...
                  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4 animate-scale-up">
                    <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <h3 className="font-extrabold text-sm text-slate-900 dark:text-white">Mark Item Unavailable</h3>
                          <p className="text-[10px] text-slate-500">Procurement stock adjustment</p>
                        </div>
                      </div>
                      <button onClick={() => setAmendingItem(null)} className="p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400">
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                    ...
                    <div className="bg-slate-50 dark:bg-slate-950 p-3 rounded-xl border border-slate-200 dark:border-slate-800 text-xs space-y-1">
                      <p className="font-extrabold text-slate-900 dark:text-slate-100">{amendingItem.item.name}</p>
                      <p className="text-[10px] text-slate-500">
                        Strength: {amendingItem.item.strength || "—"} • Pack: {amendingItem.item.packSize || "—"}
                      </p>
                      <div className="flex justify-between items-center pt-1 border-t border-slate-200/60 dark:border-slate-800 font-mono text-[11px] text-slate-700 dark:text-slate-300">
                        <span>Removing: x{amendingItem.item.quantity} units</span>
                        <span className="font-bold text-rose-600 dark:text-rose-400">-৳{amendingItem.item.subtotal?.toLocaleString()}</span>
                      </div>
                    </div>

                    <div className="space-y-2">
                      <label className="text-[11px] font-bold text-slate-700 dark:text-slate-300 block">
                        Reason for Unavailability (Optional / Fast)
                      </label>
                      <div className="flex flex-wrap gap-1.5 mb-2">
                        {["হোলসেলার স্টকে নেই", "সাপ্লায়ার শর্ট সাপ্লাই", "Procurement shortage", "Damaged batch at supplier"].map(reasonChip => (
                          <button
                            key={reasonChip}
                            type="button"
                            onClick={() => setAmendReason(reasonChip)}
                            className={`text-[10px] px-2.5 py-1 rounded-lg border font-medium transition-all ${
                              amendReason === reasonChip
                      ...
                      <input
                        ...
                        placeholder="যেমন: হোলসেলার স্টকে নেই / সাময়িক অনুপলব্ধ"
                        className="w-full bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-900 dark:text-slate-100 focus:outline-none focus:border-purple-500"
                      />
                    </div>

                    <div className="text-[11px] text-slate-500 dark:text-slate-400 bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-900/40 rounded-xl p-3">
                      💡 <strong>Operational Flow:</strong> Removing this item will automatically recalculate the invoice total for Cash on Delivery collection, regenerate the packing slip, and notify the pharmacy owner immediately.
                    </div>

                    {amendError && (
                      <div className="bg-rose-50 border border-rose-200 text-rose-600 text-xs p-2.5 rounded-xl font-medium">
                        {amendError}
                      </div>
                    )}
                    ...
                    <div className="flex gap-2 pt-2">
                      <button
                        type="button"
                        onClick={() => setAmendingItem(null)}
                        disabled={isAmending}
                        className="flex-1 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold py-2 px-4 rounded-xl text-xs cursor-pointer"
                      >
                        Cancel
                      </button>
                      <button
                        ...
                        disabled={isAmending}
                        className="flex-1 bg-rose-600 hover:bg-rose-500 text-white font-bold py-2 px-4 rounded-xl text-xs cursor-pointer flex items-center justify-center gap-1.5 shadow-md shadow-rose-600/20"
                      >
                        {isAmending ? "Recalculating..." : "Confirm & Amend Order"}
                      </button>
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-md shadow-2xl p-6 space-y-6">
            ...
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="text-slate-400 hover:text-purple-600 dark:hover:text-purple-400 p-1 rounded-lg transition-colors cursor-pointer"
                  title="Download Warehouse Packing Slip (.txt)"
                >
            ...
            <div className="space-y-2">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-2.5">
                <button
                  onClick={() => setIsPrintPreviewOpen(false)}
                  className="w-full bg-slate-100 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-850 border border-slate-200 dark:border-slate-800 text-[#14161B] dark:text-slate-400 font-bold py-2.5 px-3 rounded-xl text-xs cursor-pointer transition-all"
                >
                  Cancel
                </button>
                <button
                  onClick={handleDownloadSlip}
                  className="w-full bg-slate-800 hover:bg-slate-900 dark:bg-slate-800 dark:hover:bg-slate-700 text-white font-bold py-2.5 px-3 rounded-xl text-xs cursor-pointer flex items-center justify-center gap-1.5 shadow-sm transition-all"
                  title="Download Warehouse Packing Slip (.txt)"
                >
                  <Download className="w-4 h-4 text-purple-300" /> Download Slip
                </button>
                <button
                  onClick={handlePrintSlip}
                  className="w-full bg-purple-600 hover:bg-purple-700 text-white font-bold py-2.5 px-3 rounded-xl text-xs cursor-pointer flex items-center justify-center gap-1.5 shadow-sm transition-all"
                >
              ...
              <div className="flex items-center justify-center pt-1">
                <button
                  ...
                  className="text-[11px] text-purple-600 dark:text-purple-400 hover:underline font-semibold flex items-center gap-1 cursor-pointer"
                >
                  <FileText className="w-3.5 h-3.5" />
                  <span>Download Official Order Invoice (PDF)</span>
                </button>
              </div>
            </div>
