...
import { 
  ...
  Edit, Eye, Plus, FileText, Zap, Calendar, ArrowRight, Activity, MapPin, CheckCheck
} from "lucide-react";
...
export default function DepotDashboard({ currentUser, onLogout }: DepotDashboardProps) {
  ...
  const [activeKpiFilter, setActiveKpiFilter] = useState<"pending" | "processing" | "packed" | "dispatch" | "delivered" | "lowStock" | "expiring" | null>(null);
  ...
  const handleOrderAction = async (orderId: string, action: "accept" | "process" | "pack") => {
    try {
      const res = await apiFetch(`/api/depot/orders/${orderId}/${action}`, { method: "POST" });
      if (res.ok) {
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                  ...
                  <button
                    ...
                    className={`p-3.5 rounded-xl text-left cursor-pointer transition-all border-l-4 border-l-amber-500 bg-white dark:bg-slate-950 border border-slate-200/80 dark:border-slate-800 shadow-sm dark:shadow-none hover:shadow-md ${
                  ...
                  <button
                    ...
                    className={`p-3.5 rounded-xl text-left cursor-pointer transition-all border-l-4 border-l-purple-500 bg-white dark:bg-slate-950 border border-slate-200/80 dark:border-slate-800 shadow-sm dark:shadow-none hover:shadow-md ${
                  ...
                  <button
                    ...
                    className={`p-3.5 rounded-xl text-left cursor-pointer transition-all border-l-4 border-l-lime-500 bg-white dark:bg-slate-950 border border-slate-200/80 dark:border-slate-800 shadow-sm dark:shadow-none hover:shadow-md ${
                  ...
                  <button
                    ...
                    className={`p-3.5 rounded-xl text-left cursor-pointer transition-all border-l-4 border-l-emerald-500 bg-white dark:bg-slate-950 border border-slate-200/80 dark:border-slate-800 shadow-sm dark:shadow-none hover:shadow-md ${
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mt-1">
                      ...
                      <div className="relative w-7 h-7 shrink-0">
                        ...
                        <span className="absolute inset-0 flex items-center justify-center text-[7px] font-black text-emerald-700 dark:text-emerald-400">
                  ...
                  {/* 5. Delivered / Fulfilled History */}
                  <button
                    onClick={() => setActiveKpiFilter(activeKpiFilter === "delivered" ? null : "delivered")}
                    className={`p-3.5 rounded-xl text-left cursor-pointer transition-all border-l-4 border-l-teal-500 bg-white dark:bg-slate-950 border border-slate-200/80 dark:border-slate-800 shadow-sm dark:shadow-none hover:shadow-md col-span-2 md:col-span-1 ${
                      activeKpiFilter === "delivered" ? "ring-2 ring-teal-500 bg-teal-50/50 dark:bg-teal-950/30" : ""
                    }`}
                  >
                    <div className="flex items-center justify-between gap-1">
                      <p className="text-[10px] font-extrabold text-[#6B7280] dark:text-slate-400 uppercase tracking-wider truncate">
                      ...
                      <CheckCheck className="w-4 h-4 text-teal-600 dark:text-teal-400 shrink-0" />
                    </div>
                    <p className="text-2xl font-black mt-1 text-[#14161B] dark:text-slate-100">{stats.delivered}</p>
                    <p className="text-[8px] text-teal-700 dark:text-teal-400 mt-1 uppercase font-bold">
                      {activeKpiFilter === "delivered" ? "● Active Filter" : "Past Archive"}
                    </p>
                  </button>
                </div>
              </div>
              ...
              <div className="lg:col-span-2 space-y-4">
                ...
                {loading ? (
                        ...
                        {orders.length > 0 && (
                          <div className="bg-white dark:bg-slate-950 p-4 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-3">
                            <div className="flex items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-800">
                              <h3 className="text-xs font-bold text-[#14161B] dark:text-slate-200 uppercase tracking-wider">
                                Recent Fulfilled / Dispatched Orders ({orders.slice(0, 5).length})
                              </h3>
                              <button
                                onClick={() => setActiveRoute("/depot/orders")}
                                className="text-[10px] text-purple-700 dark:text-purple-400 font-bold hover:underline cursor-pointer flex items-center gap-1"
                              >
                            ...
                            <div className="space-y-2">
                              {orders.slice(0, 5).map(order => (
                                <div key={order.id} className="p-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200/80 dark:border-slate-800 flex items-center justify-between text-xs">
                                  <div>
                                    <div className="flex items-center gap-2">
                                      <span className="font-extrabold text-[#14161B] dark:text-slate-200">#{order.readableId || order.id.substring(0, 8)}</span>
                                      <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase ${
                                        order.status === "Delivered" ? "bg-teal-100 text-teal-800 dark:bg-teal-950/80 dark:text-teal-300" :
                                        order.status === "Out for Delivery" ? "bg-emerald-100 text-emerald-800 dark:bg-emerald-950/80 dark:text-emerald-300" :
                                        "bg-purple-100 text-purple-800 dark:bg-purple-950/80 dark:text-purple-300"
                                      }`}>
                                        {order.status}
                                      </span>
                                    </div>
                                    <p className="text-[10px] text-[#6B7280] dark:text-slate-400 font-medium mt-0.5">
                                      {order.pharmacyName || "Pharmacy"} • {order.items?.length || 0} items • Total: ৳{order.totalAmount}
                                    </p>
                                  </div>
                                  <button
                                    onClick={() => setActiveRoute("/depot/orders")}
                                    className="p-1.5 text-purple-600 dark:text-purple-400 hover:bg-purple-50 dark:hover:bg-purple-950/30 rounded-lg transition-all"
                                  >
                  ...
                  <div className="space-y-3">
                    ...
                    {(activeKpiFilter === "pending" || activeKpiFilter === "processing" || activeKpiFilter === "packed" || activeKpiFilter === "dispatch" || activeKpiFilter === "delivered") && (
                      orders.filter(o => {
                        ...
                        if (activeKpiFilter === "delivered") return o.status === "Delivered";
                        return false;
                      }).length === 0 ? (
                        <ChainLinkEmptyState 
