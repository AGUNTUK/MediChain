...
import { ArrowLeft, Check, Compass, Truck, RefreshCw, Layers, Calendar, Phone, KeyRound, Package, History } from "lucide-react";
...
export default function OrderTracking({ orderId, userRole, onBack, onRefreshStats }: OrderTrackingProps) {
  ...
  useEffect(() => {
    ...
    socket.on("order_amended", (data: any) => {
      if (data && data.order) {
        setOrder(data.order);
      }
      onRefreshStats();
  ...
  return (
    <div className="w-full h-full bg-brand-bg flex flex-col justify-between select-none overflow-y-auto">
      ...
      <div className="p-4 space-y-4">
            ...
            {(order.items || []).map((item, idx) => (
              <div key={idx} className="flex justify-between items-center text-xs py-1 border-b border-slate-50 last:border-0">
                <div>
                  ...
                  <p className="text-[10px] text-slate-400">{item.strength || ""} • {item.quantity} Box</p>
                </div>
                <div className="text-right">
                  <p className="font-mono font-bold text-slate-700">৳{item.subtotal?.toLocaleString()}</p>
                  <p className="text-[9px] text-slate-400">৳{item.sellingPrice} ea</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Procurement Amendment Audit Card */}
        {order.amendments && order.amendments.length > 0 && (
          <div className="bg-amber-50/90 border border-amber-200/90 rounded-2xl p-4 space-y-2 shadow-3xs">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-black text-amber-900 uppercase tracking-wider flex items-center gap-1.5">
                <History className="w-3.5 h-3.5 text-amber-600" />
                Procurement Adjustments ({order.amendments.length})
              </span>
              <span className="text-[9px] font-bold text-amber-800 bg-amber-200/70 px-2 py-0.5 rounded-full">
                COD Recalculated
              </span>
            </div>
            <p className="text-[10px] text-amber-800 leading-relaxed">
              Wholesaler stock was unavailable for the following line items during pre-packing collection. Your invoice was automatically adjusted:
            </p>
            <div className="space-y-1.5 pt-1">
              {order.amendments.map((amend, aIdx) => (
                <div key={aIdx} className="bg-white/80 border border-amber-200/60 rounded-xl p-2 text-xs flex justify-between items-center">
                  <div>
                    <p className="font-bold text-slate-800 line-through">{amend.productName}</p>
                    <p className="text-[10px] text-amber-700 font-medium">Reason: {amend.reason || "Wholesaler stock shortage"}</p>
                  </div>
                  <div className="text-right">
                    <span className="text-[10px] font-bold text-rose-600 bg-rose-50 px-2 py-0.5 rounded-md">
                      -{amend.removedQuantity} Box
                    </span>
                  </div>
                </div>
              ))}
