...
export default function ModernInvoiceModal({ order, pharmacy, onClose }: ModernInvoiceModalProps) {
  ...
  const [invoiceHeight, setInvoiceHeight] = useState<number>(1150);
  ...
  useEffect(() => {
    const updateDimensions = () => {
      ...
      if (invoiceRef.current) {
        setInvoiceHeight(invoiceRef.current.scrollHeight || invoiceRef.current.offsetHeight || 1150);
      }
      if (zoomMode === "fit") {
        // Fit completely inside the container width with a comfortable padding
  ...
  // Delivery Charge: default 40৳ for all orders
  ...
  items.forEach(it => {
    const itemSellingPrice = it.sellingPrice || 0;
    const itemMrp = it.mrp || (itemSellingPrice * 1.25);
    const qty = it.quantity || 1;
    calculatedMrp += itemMrp * qty;
    calculatedSubtotal += (it.subtotal !== undefined ? it.subtotal : itemSellingPrice * qty);
  });

  const totalMrp = order.totalMrp && order.totalMrp > 0 ? order.totalMrp : (calculatedMrp || calculatedSubtotal);
  
  // Calculate subtotal (medicines total)
  const subtotal = calculatedSubtotal > 0 
    ? calculatedSubtotal 
    : Math.max(0, (order.totalAmount || 0) - deliveryCharge);

  // Grand Total: ensures default 40৳ delivery charge is automatically included in the invoice
  const grandTotal = (order.totalAmount && Math.abs(order.totalAmount - (subtotal + deliveryCharge)) < 1)
    ? order.totalAmount
    : (subtotal + deliveryCharge);

  const totalSavings = order.totalSavings && order.totalSavings > 0 
    ? order.totalSavings 
      ...
      >
        ...
        <div className="no-print bg-slate-900 text-white px-3 sm:px-5 py-3 flex flex-wrap items-center justify-between gap-2.5 border-b border-slate-800">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-purple-600/30 border border-purple-500/40 flex items-center justify-center text-purple-300 font-bold text-sm shrink-0">
              ৳
            </div>
            <div>
              <div className="text-xs font-black tracking-wide flex items-center gap-1.5 sm:gap-2">
                <span className="truncate max-w-[140px] sm:max-w-none">চালান রসিদ / INVOICE</span>
              ...
              <p className="text-[10px] text-slate-400 hidden sm:block">
                অফিসিয়াল এ-ফোর চালান (Standard A4 Format)
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap ml-auto">
            ...
            <div className="flex items-center bg-slate-800/90 rounded-xl p-0.5 border border-slate-700/80 text-[11px]">
              <button
                ...
                title="পুরো চালান এক নজরে দেখুন (Fit to Screen)"
              >
                <Maximize2 className="w-3 h-3" />
                <span className="text-[10px] sm:text-xs">ফিট (Fit)</span>
              </button>
              ...
              <button
                ...
                title="আসল ১০০% সাইজে দেখুন (100% Size)"
              >
            ...
            <button
              onClick={handlePrint}
              className="bg-purple-600 hover:bg-purple-500 text-white text-xs font-bold py-1.5 px-3 sm:px-3.5 rounded-xl flex items-center gap-1 transition-all shadow-md shadow-purple-600/20 cursor-pointer active:scale-95"
              title="প্রিন্ট করুন"
            >
              <Printer className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">প্রিন্ট (Print)</span>
            </button>
            ...
            <button
              ...
              title="পিডিএফ চালান ডাউনলোড করুন"
            >
        ...
        >
          ...
          >
            <div
              ...
              className="bg-white text-slate-900 shadow-xl rounded-2xl p-6 sm:p-8 font-sans border border-slate-200 space-y-5"
              style={{
            ...
            >
              ...
              <div className="flex flex-row justify-between items-start border-b border-slate-200 pb-5 gap-4">
                ...
                <div className="text-xs text-slate-600 text-center leading-relaxed px-2 flex-1 max-w-[280px]">
              ...
              {/* 2. THREE INFO CARDS (Always 3 Columns in A4 Canvas) */}
              <div className="grid grid-cols-3 gap-3.5 avoid-page-break">
                {/* Card 1: BILLED TO */}
              ...
              {/* 4. TERMS, SIGNATURE & FINANCIAL TOTALS GRID (Always 12 Columns in A4 Canvas) */}
              <div className="grid grid-cols-12 gap-4 items-start pt-2 avoid-page-break">
                ...
                <div className="col-span-3 flex flex-col items-center justify-end h-full pt-4 text-center">
                ...
                <div className="col-span-4 rounded-xl border border-slate-200 overflow-hidden shadow-2xs">
                  <div className="divide-y divide-slate-100 text-[11px] bg-white">
                    ...
                    <div className="flex justify-between py-1.5 px-3 text-slate-700">
                      <span className="font-bold">Subtotal (Medicines)</span>
              ...
              {/* 5. TRUST BADGES / VALUE PROPOSITION BAR (Always 4 Columns in A4 Canvas) */}
              <div className="rounded-2xl border border-slate-200 bg-[#FAF8FD] p-3.5 grid grid-cols-4 gap-2.5 items-center text-slate-800 avoid-page-break">
                {/* Box 1: Thanks */}
                <div className="flex items-center gap-2.5 border-r border-slate-200 pr-2">
                  ...
                  <div>
                    ...
                    <div className="text-[8.5px] font-bold text-[#45A834] mt-0.5">
                      100% অরিজিনাল • সেরা দাম
                    </div>
                  </div>
                </div>

                {/* Box 2: 100% Original Medicine */}
                <div className="flex items-center gap-2 justify-center border-r border-slate-200 px-2">
