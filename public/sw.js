...
const MAX_CACHE_ENTRIES = 150; // Cap cache entries to prevent mobile browser storage exhaustion (J3)
...
async function trimCache(cacheName, maxItems) {
  try {
    ...
    console.warn('[MediChain SW] Cache trim error:', err);
  }
}
        ...
        return fetch(event.request).then(async (networkResponse) => {
          if (networkResponse.ok && (networkResponse.type === 'basic' || networkResponse.type === 'cors')) {
            try {
              ...
              console.warn('[MediChain SW] Asset caching failed (quota full or abort):', cacheErr);
            }
          }
          return networkResponse;
        }).catch((err) => {
